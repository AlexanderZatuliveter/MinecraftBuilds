        ...
        for z in range(10, length - 10, 15):
            ...
            for y in range(height - 2):
                ...
                if y == height - 4:
                    for dx in [-1, 1]:
                        for dz in [-1, 1]:
                            mc.setBlock("chiseled stone bricks", 
...
def build_castle_walls(base_pos, size, height=12):
    ...
    # Main walls
    wall_positions = [
        (Vec3(base_pos.x, base_pos.y, base_pos.z), Vec3(base_pos.x + size, base_pos.y + height, base_pos.z + 3)),  # North
        (Vec3(base_pos.x, base_pos.y, base_pos.z + size - 3), Vec3(base_pos.x + size, base_pos.y + height, base_pos.z + size)),  # South
        (Vec3(base_pos.x, base_pos.y, base_pos.z), Vec3(base_pos.x + 3, base_pos.y + height, base_pos.z + size)),  # West
        (Vec3(base_pos.x + size - 3, base_pos.y, base_pos.z), Vec3(base_pos.x + size, base_pos.y + height, base_pos.z + size))  # East
    ]
    
    for pos1, pos2 in wall_positions:
        build_filled_box("stone bricks", pos1, pos2)
    
    # Battlements on walls
    for i, (pos1, pos2) in enumerate(wall_positions):
        if i < 2:  # North and South walls
            build_battlements(Vec3(pos1.x, pos2.y, pos1.z), size, 3)
        else:  # West and East walls
            build_battlements(Vec3(pos1.x, pos2.y, pos1.z), 3, size)
...
#     Vec3(castle_base.x, castle_base.y + 40, castle_base.z)
# ]
