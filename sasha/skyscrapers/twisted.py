...
def build_twisted_skyscraper():
    ...
    materials = {
        ...
        'windows': 'glass',
        'window frame': 'iron bars',
        'interior floor': 'polished andesite',
        ...
        for x in range(int(base_x - current_size // 2), int(base_x + current_size // 2 + 1)):
            for z in range(int(base_z - current_size // 2), int(base_z + current_size // 2 + 1)):
                ...
                if abs(rotated_x) <= current_size // 2 and abs(rotated_z) <= current_size // 2:
                    wrap.set_block(materials['interior floor'], Vec3(x, y_level, z))
            ...
            for wall_y in range(y_level + 1, y_level + floor_height - 1):
                ...
                if is_window_level and is_window_position:
                    ...
                    if (wall_y - y_level) in [2, 4]:
                        wrap.set_block(materials['window frame'], Vec3(wall_x, wall_y, wall_z))
                else:
            ...
            for x in range(int(base_x - current_size // 4), int(base_x + current_size // 4 + 1)):
                for z in range(int(base_z - current_size // 4), int(base_z + current_size // 4 + 1)):
                    ...
                    if abs(rotated_x) <= current_size // 4 and abs(rotated_z) <= current_size // 4:
                        wrap.set_block('grass block', Vec3(x, y_level + 1, z))
                        if (x + z) % 4 == 0:
                            wrap.set_block('oak leaves', Vec3(x, y_level + 2, z))
        ...
        for floor_num in range(1, floors):
            if floor_num % 7 == 0:  # Balcony every 7 floors
                ...
                for bal_x, bal_z in balcony_points:
                    ...
                    # Balcony railing
                    wrap.set_block(materials['window frame'], Vec3(balcony_x, y_level + 2, balcony_z))

    # Build the complete skyscraper
    mc.postToChat("Building foundation...")
    build_foundation()

    mc.postToChat("Building floors...")
    for floor in range(floors):
        y_level = base_y + floor * floor_height
        build_floor(floor, y_level)
        if floor % 5 == 0:
            mc.postToChat(f"Completed floor {floor}/{floors}")

    mc.postToChat("Building roof and spire...")
    build_roof()

    mc.postToChat("Adding external features...")
    build_external_features()

    mc.postToChat("Rendering to Minecraft...")
    wrap.draw()

    mc.postToChat(f"Twisted skyscraper completed! {floors} floors with full 180° twist.")
    mc.postToChat("Features included:")
    mc.postToChat("- Gradual twist from base to top")
    mc.postToChat("- Individual floor details")
    mc.postToChat("- Windows with frames")
    mc.postToChat("- Corner pillars")
    mc.postToChat("- Observation deck")
    mc.postToChat("- Sky garden level")
    mc.postToChat("- Balconies every 7 floors")
    mc.postToChat("- Decorative spire with beacon")
